...
contract('SeizableToken', function (accounts) {
  ...
  it('should seize 1 from account', async function () {
    await token.seize(accounts[1], 1);
  ...
  it('should seize everything from account', async function () {
    await token.seize(accounts[1], 50);
  ...
  it('should increase allTimeSeize value', async function () {
    await token.seize(accounts[1], 10);
  ...
  it('should log an event when seizing 2', async function () {
    const receipt = await token.seize(accounts[1], 2);
    assert.equal(receipt.logs.length, 1);
    assert.equal(receipt.logs[0].event, 'Seized');
  ...
  it('should throw an error trying to seize more than account have', async function () {
    await assertThrow(token.seize(accounts[1], 1000));
  });

  it('should revert trying to seize owner (self)', async function () {
    await assertRevert(token.seize(accounts[0], 1000));
  });

  it('should not allow other than owner to issue', async function () {
    await assertRevert(token.seize(accounts[1], 1000, { from: accounts[1] }));
  });
});
