 ...
 *
 * Error messages
 * E01: No plans are configured
 * E02: Historical steps must happened in the past
 * E03: New step must be no earlier the previous one
 * E04: There is no transitions to update
 * E05: Transition update must be in the future
 * E06: Current step is already the last step
 * E07: The current step has already a transition
*/
contract StateMachine is IStateMachine, Ownable {
    ...
    uint256 _transitionDelay) internal onlyOwner returns (uint256)
  {
    ...
    require(_stepTime < currentTime(), "E02");
    if (steps.length > 0) {
      require(_stepTime > steps[steps.length-1].stepTime, "E03");
    }
    ...
    uint256 _transitionDelay) internal onlyOwner
  {
    ...
    if (_transitionEndTime > 0) {
      require(steps[currentStepId].transitionEndTime > 0, "E04");
      require(_transitionEndTime > currentTime(), "E05");
      steps[currentStepId].transitionEndTime = _transitionEndTime;
    }

    if (_transitionDelay > 0) {
      require(steps[currentStepId].transitionDelay > 0, "E04");
  ...
  function nextStep() internal {
    ...
    require(
      ...
      steps[currentStepId].transitionDelay == 0, "E07");
